...
def get_all_recent(days = 120,work_n= 15,money_flow = False,read_hdf = True):
    """
...
def _get_all_recent(days_or_days_list,work_n= 15,money_flow = True):
...
class processbar:
    "print processbar "
    last_func_name = ""
    def __init__(self, print_str, i, total, func_name = ""):
        end_symbol = ""
        if self.last_func_name != func_name:
            self.last_func_name = func_name
            end_symbol = "\n"
        i = i if i < total else total
        if i == total:
            end_symbol = "\n"
...
def _allprep_data_storage(days = 120, update = 1):
...
def days_allprep_data(ts_code,time_stamp ="",days = 121,**kwargs):
...
def get_ts_data(ts_code,time_stamp="",money_flow = True):
    """得到单个股票的信息"""
    pro_bar = get_df_three_times(ts.pro_bar)
...
if __name__ == '__main__':
    df = moneyflow_onedays_df(trade_date = "20190527")
    # money_flow_dfs = _threading_load_recent(moneyflow_onedays_df, work_n=3, trade_date=['20190627'])
    # moneyflow_df = pd.concat(money_flow_dfs, axis=0, sort=True)
    # df = days_allprep_data("300516.SZ")
    print(df)
